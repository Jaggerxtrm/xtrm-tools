...
| Section | Summary |
|---|---|
| [Hook lifecycle](#hook-lifecycle) | Hooks fire at specific points during a Claude Code session |
| [Configuration](#configuration) | Claude Code hooks are configured in your [settings files](/en/settings): |
| [Prompt-Based Hooks](#prompt-based-hooks) | In addition to bash command hooks (`type: "command"`), Claude Code supports prompt-based hooks (`type: "prompt"`) that u |
| [Hook Events](#hook-events) | Runs after Claude creates tool parameters and before processing the tool call |
| [Hook Input](#hook-input) | Hooks receive JSON data via stdin containing session information and |
| [Hook Output](#hook-output) | There are two mutually exclusive ways for hooks to return output back to Claude Code |
| [Working with MCP Tools](#working-with-mcp-tools) | Claude Code hooks work seamlessly with |
| [Examples](#examples) | <Tip> |
| [Security Considerations](#security-considerations) | **USE AT YOUR OWN RISK**: Claude Code hooks execute arbitrary shell commands on |
| [Hook Execution Details](#hook-execution-details) | * **Timeout**: 60-second execution limit by default, configurable per command |
| [Debugging](#debugging) | If your hooks aren't working: |
<!-- END INDEX -->
> ## Documentation Index
