...
title: Untitled
date_created: Thursday, January 29th 2026, 1:28:47 pm
date_modified: Thursday, January 29th 2026, 1:28:56 pm
---
scope: reference
category: reference
version: 1.0.0
...
| Section | Summary |
|---|---|
| [Model compatibility](#model-compatibility) | Programmatic tool calling is available on the following models: |
| [Quick start](#quick-start) | Here's a simple example where Claude programmatically queries a database multiple times and aggregates results: |
| [How programmatic tool calling works](#how-programmatic-tool-calling-works) | When you configure a tool to be callable from code execution and Claude decides to use that tool: |
| [Core concepts](#core-concepts) | The `allowed_callers` field specifies which contexts can invoke a tool: |
| [Example workflow](#example-workflow) | Here's how a complete programmatic tool calling flow works: |
| [Advanced patterns](#advanced-patterns) | Claude can write code that processes multiple items efficiently: |
| [Response format](#response-format) | When code execution calls a tool: |
| [Error handling](#error-handling) | If your tool takes too long to respond, the code execution will receive a `TimeoutError` |
| [Constraints and limitations](#constraints-and-limitations) | The following tools cannot currently be called programmatically, but support may be added in future releases: |
| [Token efficiency](#token-efficiency) | Programmatic tool calling can significantly reduce token consumption: |
| [Usage and pricing](#usage-and-pricing) | Programmatic tool calling uses the same pricing as code execution |
| [Best practices](#best-practices) | **Good use cases:** |
| [Troubleshooting](#troubleshooting) | **"Tool not allowed" error** |
| [Why programmatic tool calling works](#why-programmatic-tool-calling-works) | Claude's training includes extensive exposure to code, making it effective at reasoning through and chaining function ca |
| [Alternative implementations](#alternative-implementations) | Programmatic tool calling is a generalizable pattern that can be implemented outside of Anthropic's managed code executi |
| [Related features](#related-features) | <CardGroup cols={2}> |
<!-- END INDEX -->
