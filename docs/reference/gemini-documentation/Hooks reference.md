...
title: Untitled
date_created: Monday, February 2nd 2026, 1:48:25 pm
date_modified: Monday, February 2nd 2026, 1:48:35 pm
---
scope: reference
category: reference
version: 1.0.0
...
| Section | Summary |
|---|---|
| [Global hook mechanics](#global-hook-mechanics) | `stderr` for logs and feedback |
| [Configuration schema](#configuration-schema) | Hooks are defined in `settings |
| [Base input schema](#base-input-schema) | All hooks receive these common fields via `stdin`: |
| [Common output fields](#common-output-fields) | Most hooks support these fields in their `stdout` JSON: |
| [Tool hooks](#tool-hooks) | For `BeforeTool` and `AfterTool` events, the `matcher` field in your settings is |
| [Agent hooks](#agent-hooks) | Fires after a user submits a prompt, but before the agent begins planning |
| [Model hooks](#model-hooks) | Fires before sending a request to the LLM |
| [Lifecycle & system hooks](#lifecycle-&-system-hooks) | Fires on application startup, resuming a session, or after a `/clear` command |
| [Stable Model API](#stable-model-api) | Gemini CLI uses these structures to ensure hooks don't break across SDK updates |
<!-- END INDEX -->
# Hooks reference
