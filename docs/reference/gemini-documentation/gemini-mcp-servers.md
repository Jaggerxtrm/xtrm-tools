...
| Section | Summary |
|---|---|
| [What is an MCP server?](#what-is-an-mcp-server?) | An MCP server is an application that exposes tools and resources to the Gemini |
| [Core integration architecture](#core-integration-architecture) | The Gemini CLI integrates with MCP servers through a sophisticated discovery and |
| [Working with MCP resources](#working-with-mcp-resources) | Some MCP servers expose contextual “resources” in addition to the tools and |
| [How to set up your MCP server](#how-to-set-up-your-mcp-server) | The Gemini CLI uses the `mcpServers` configuration in your `settings |
| [Discovery process deep dive](#discovery-process-deep-dive) | When the Gemini CLI starts, it performs MCP server discovery through the |
| [Tool execution flow](#tool-execution-flow) | When the Gemini model decides to use an MCP tool, the following execution flow |
| [How to interact with your MCP server](#how-to-interact-with-your-mcp-server) | The `/mcp` command provides comprehensive information about your MCP server |
| [Status monitoring and troubleshooting](#status-monitoring-and-troubleshooting) | The MCP integration tracks several states: |
| [Important notes](#important-notes) | cautiously and only for servers you completely control |
| [Returning rich content from tools](#returning-rich-content-from-tools) | MCP tools are not limited to returning simple text |
| [MCP prompts as slash commands](#mcp-prompts-as-slash-commands) | In addition to tools, MCP servers can expose predefined prompts that can be |
| [Managing MCP servers with `gemini mcp`](#managing-mcp-servers-with-`gemini-mcp`) | While you can always configure MCP servers by manually editing your |
| [Instructions](#instructions) | Gemini CLI supports |
<!-- END INDEX -->
