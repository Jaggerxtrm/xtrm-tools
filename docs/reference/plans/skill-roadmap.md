...
| Section | Summary |
|---|---|
| [`prompt-improving` Skill](#`prompt-improving`-skill) | **Goal:** Enable batch prompt improvement via code execution for multi-prompt workflows |
| [`delegating` Skill](#`delegating`-skill) | **Goal:** Track delegation success/failure rates per profile |
| [Cross-Skill Features](#cross-skill-features) | **Goal:** Proactive skill suggestions beyond `/prompt-improving` and `/ccs` |
| [Deprecation Timeline](#deprecation-timeline) | **Reasoning:** 90% token overhead for <5% usage |
| [MCP Integration](#mcp-integration) | **Goal:** Expose skills as MCP tools for cross-client compatibility |
| [Version History](#version-history) | Suggest roadmap items by creating issues in: |
| [Contributing](#contributing) | Suggest roadmap items by creating issues in: |
...
- `docs/reference/plans/skill-roadmap.md` (this file)
