...
| Section | Summary |
|---|---|
| [Quick Start](#quick-start) | Launch a new session or re-attach to an existing one |
| [What Happens on Launch](#what-happens-on-launch) | Steps executed by `xt claude` / `xt pi`, including session metadata write |
| [Re-attaching to a Session](#re-attaching-to-a-session) | `xt attach` resumes a closed Claude or Pi session in an existing worktree |
...
| [Managing Worktrees](#managing-worktrees) | `xt worktree list` shows runtime, last activity, last commit, and resume hint |
